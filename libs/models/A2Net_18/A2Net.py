...
class NeighborFeatureAggregation(nn.Module):
    ...
    def forward(self, c2, c3, c4, c5):
        ...
        c3_s2 = F.interpolate(c3_s2, size=c2.size()[2:], mode='bilinear', align_corners=True)
        ...
        c4_s3 = F.interpolate(c4_s3, size=c3.size()[2:], mode='bilinear', align_corners=True)
        ...
        c5_s4 = F.interpolate(c5_s4, size=c4.size()[2:], mode='bilinear', align_corners=True)
...
class Decoder(nn.Module):
    ...
    def forward(self, d2, d3, d4, d5):
        ...
        p4 = self.conv_p4(d4 + F.interpolate(p5, size=d4.size()[2:], mode='bilinear', align_corners=True))

        p4, mask_p4 = self.sam_p4(p4)
        p3 = self.conv_p3(d3 + F.interpolate(p4, size=d3.size()[2:], mode='bilinear', align_corners=True))

        p3, mask_p3 = self.sam_p3(p3)
        p2 = self.conv_p2(d2 + F.interpolate(p3, size=d2.size()[2:], mode='bilinear', align_corners=True))
        mask_p2 = self.cls(p2)
...
class Decoder_Segmentation(nn.Module):
    def __init__(self, mid_d=320, output_nc=7):
        ...
        self.cls = nn.Sequential(
            nn.Conv2d(self.mid_d + 1, self.mid_d, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(self.mid_d),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.mid_d, output_nc, kernel_size=1)
        )

    def forward(self, d2, d3, d4, d5, mask_p2):
        # high-level
        p4 = self.conv_p4(d4 + F.interpolate(d5, size=d4.size()[2:], mode='bilinear', align_corners=True))
        p3 = self.conv_p3(d3 + F.interpolate(p4, size=d3.size()[2:], mode='bilinear', align_corners=True))
        p2 = self.conv_p2(d2 + F.interpolate(p3, size=d2.size()[2:], mode='bilinear', align_corners=True))
        mask = self.cls(torch.cat([p2, mask_p2], dim=1))
...
class BaseNet(nn.Module):
    def __init__(self, bc_token_length=1, sc_token_length=7):
        ...
        self.backbone = timm.create_model('resnet18d', features_only=True, pretrained=True)
    ...
    def forward(self, x1, x2):
        ...
        mask_x1 = self.sc_decoder(x1_2, x1_3, x1_4, x1_5, mask_p2)
        mask_x2 = self.sc_decoder(x2_2, x2_3, x2_4, x2_5, mask_p2)

        # change map
        mask_x1 = F.interpolate(mask_x1, size=x1.size()[2:], mode='bilinear', align_corners=True)
        mask_x2 = F.interpolate(mask_x2, size=x1.size()[2:], mode='bilinear', align_corners=True)
        mask_p2 = F.interpolate(mask_p2, size=x1.size()[2:], mode='bilinear', align_corners=True)
